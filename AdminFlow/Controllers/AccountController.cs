...
using AdminFlow.Models;
using AdminFlow.ViewModels;
using Microsoft.AspNetCore.Identity;
using Microsoft.AspNetCore.Mvc;

namespace AdminFlow.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var url = Url.Action("ConfirmEmail", "Acount", new { user.Id, token });

                    //email
                    await _emailSender.SendEmailAsync(user.Email, "Hesap Onayı", $"Lütfen Email hesabınızı onaylamak için linke <a href='http://localhost:5092{url}'>tıklayınız</a>.");
        ...
        public async Task<IActionResult> ForgotPassword(string Email)
        {
            ...
            if (user == null)
            {
                TempData["message"] = "Eposta adresiyle eşleşen bşr kayıt yok!";
                return View();
            }
            ...
            await _emailSender.SendEmailAsync(Email, "Parola Sıfırlama", $"Parolanızı yenilemek için linke <a href='http://localhost:5092{url}'>tıklayınız</a>.");
